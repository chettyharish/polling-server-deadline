...
static inline int sched_poll_capacity(struct task_struct *p, ktime_t now)
{
	...
	printk(KERN_ERR " POLL FUNCTION : \t%lld\t%lld\n", p->dl.sched_poll_initial_budget.tv64,p->dl.sched_poll_current_usage.tv64	);
...
static inline void sched_poll_throttle_control(struct task_struct *p, ktime_t now)
{
	...
	if(sched_poll_capacity(p, now) <= 0){
		if(dl_se->dl_throttled==0)
			status_changed =1;

		if(status_changed==1){
			printk(KERN_ERR " POLL FUNCTION : throttled status_changed = %d \t orig : %d \t new = %d \n",status_changed,dl_se->dl_throttled,1);
			dl_se->dl_throttled=1;
			dl_se->runtime=0;
			if (dl_runtime_exceeded(rq, dl_se)) {
			/*CHANGES END HERE*/
				__dequeue_task_dl(rq, p, 0);
				if (likely(start_dl_timer(dl_se, p->dl.dl_boosted)))
					dl_se->dl_throttled = 1;
				else
					enqueue_task_dl(rq, p, ENQUEUE_REPLENISH);
				if (!is_leftmost(p, &rq->dl))
					resched_task(p);
			}
		}
	}
	else{
		if(dl_se->dl_throttled==1)
			status_changed =1 ;


		if(status_changed==1){
			printk(KERN_ERR " POLL FUNCTION : released status_changed = %d \t orig : %d \t new = %d \n",status_changed,dl_se->dl_throttled,0);
			dl_se->dl_throttled=0;
			if (p->on_rq) {
				printk(KERN_ERR " POLL FUNCTION : P ON RQ \n");
				enqueue_task_dl(rq, p, ENQUEUE_REPLENISH);
			...
			#ifdef CONFIG_SMP
					printk(KERN_ERR " POLL FUNCTION : IS PUSHABLE \n");
	...
	if(status_changed==1){
		...
		printk(KERN_ERR " POLL FUNCTION : \t EVERYTHING SAME\n");
	}
...
static int sched_poll_fwd_repl_timer(struct task_struct *p, ktime_t now)
{
	...
	if (ktime_compare(hrtimer_get_expires(timer), now) > 0) {
		...
		printk(KERN_ERR " POLL FUNCTION Timer already set: \t%lld, \t %lld\t %d\n",  now.tv64 ,hrtimer_get_expires(timer).tv64,ktime_compare(hrtimer_get_expires(timer), now));
		return 0;
	}
	...
	if(ktime_compare(p->dl.sched_poll_current_usage,p->dl.sched_poll_replenish_period)>0){
		/*Error as usage has exceeded replenish period -> IMPOSSIBLE condition*/
		printk(KERN_ERR " POLL FUNCTION Danger timer crash imminent \n");
	}else{
		/*Within limits*/
		printk(KERN_ERR " POLL FUNCTION Under limit \n");
		hrtimer_set_expires(timer,ktime_add(ktime_sub(p->dl.sched_poll_replenish_period,p->dl.sched_poll_current_usage),now));
	}


	printk(KERN_ERR " FINALLY ");
	printk(KERN_ERR " POLL FUNCTION : \t%lld, \t %lld\t %lld\n",  now.tv64 ,hrtimer_get_expires(timer).tv64,p->dl.sched_poll_current_usage.tv64);
...
select_task_rq_dl(struct task_struct *p, int cpu, int sd_flag, int flags)
{
	 ...
	 */
	if(poll_task(curr))
		if(curr->dl.dl_throttled==1)
			printk(KERN_ERR " POLL FUNCTION : \t WAS THROTTLED");

	if(poll_task(p))
		if(p->dl.dl_throttled==1)
			goto unlock;
...
static void task_dead_dl(struct task_struct *p)
{
	...
	hrtimer_cancel(&p->dl.sched_poll_replenish_timer);
...
static int push_dl_task(struct rq *rq)
{
	...
	if (!rq->dl.overloaded)
		return 0;
	printk(KERN_ERR " POLL FUNCTION : DL not overloaded" );

	next_task = pick_next_pushable_dl_task(rq);
	if (!next_task)
		return 0;
	printk(KERN_ERR " POLL FUNCTION : NOT present" );
retry:
	if (unlikely(next_task == rq->curr)) {
		WARN_ON(1);
		return 0;
	}
	printk(KERN_ERR " POLL FUNCTION : NEXT==CURR" );
	/*
	...
	printk(KERN_ERR " POLL FUNCTION : Starting preemption" );
	...
	printk(KERN_ERR " POLL FUNCTION : Process task" );
...
out:
	put_task_struct(next_task);
	printk(KERN_ERR " POLL FUNCTION : Task IN" );
	return 1;
}
...
static void task_woken_dl(struct rq *rq, struct task_struct *p)
{
	printk(KERN_ERR " POLL FUNCTION : \t%s\n", __func__);
		...
		if (!task_running(rq, p) && !test_tsk_need_resched(rq->curr) && has_pushable_dl_tasks(rq) && p->nr_cpus_allowed > 1 && dl_task(rq->curr) && (rq->curr->nr_cpus_allowed < 2 ||dl_entity_preempt(&rq->curr->dl, &p->dl))) {
				printk(KERN_ERR " POLL FUNCTION : DL TASK");
//				if(!(p->dl.dl_throttled==1 && poll_task(p))){
					push_dl_tasks(rq);
//				}
		}

		printk(KERN_ERR " POLL FUNCTION : WOKEN END");
...
enum hrtimer_restart sched_poll_replenish_cb(struct hrtimer *timer)
{
	...
	if (ktime_compare(now, ns_to_ktime(0)) == -1)
		printk(KERN_ERR "POLL FUNCTION : now is negative\n");
              ...
              struct task_struct *next)
{
	...
	if (prev->policy == SCHED_POLL) {
		...
		sched_poll_budget_check(rq, prev, now, !prev->on_rq);
	}
}
